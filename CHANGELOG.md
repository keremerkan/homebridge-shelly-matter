...
- **Shelly Gas** ([#10](https://github.com/keremerkan/homebridge-shelly-matter/issues/10)): Matter has no gas detector type, so the new per-device `gasAlarm` option exposes the detector's alarm as a smoke alarm or a carbon monoxide alarm (mild gas = warning, heavy gas = critical, self-test shown as testing). Not exposed by default.
