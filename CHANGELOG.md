...
- Settings table: each device shows its transport (CoIoT, WebSocket, RPC over UDP; whether it sleeps) and its host links to the device's web page; a device configured for UDP while the plugin option is off (or the reverse) is flagged.
- **RPC over UDP** for Gen 2+ devices (`rpcOverUdp`, off by default): devices configured to send their updates to `<homebridge-ip>:8585` report over UDP instead of a WebSocket, the simplest transport for Gen 2+ battery devices (verified on a Plus Plug S).
- Shelly protocol layer synced to matterbridge-shelly 2.6.0: BLU devices on gateway firmware 2.0.0+ are no longer rejected (needed for the upcoming BLU support), Gen 1 temperatures reported in Fahrenheit are converted correctly, BLU Motion ZB recognized.
- **Smoke sensors** (Shelly Plus Smoke, Shelly Smoke Gen 1): exposed as a Matter smoke alarm with battery level (mapped, not yet hardware-confirmed).
- **Shelly Gas** ([#10](https://github.com/keremerkan/homebridge-shelly-matter/issues/10), confirmed on hardware as a CO alarm): Matter has no gas detector type, so the new per-device `gasAlarm` option exposes the detector's alarm as a smoke alarm or a carbon monoxide alarm (mild gas = warning, heavy gas = critical, self-test shown as testing). Not exposed by default.
- **Shelly Door/Window 2** ([#10](https://github.com/keremerkan/homebridge-shelly-matter/issues/10), confirmed on hardware): contact (open/closed), light level, temperature and battery. The vibration (impact) detection can be exposed as a motion sensor with the new per-device `vibrationAsMotion` option (off by default: Matter has no vibration sensor type, so it is a deliberate re-mapping); an impact shows as motion for 10 seconds (each new impact restarts the window), since the sleeping sensor never reports the end of it. The Door/Window 1 follows the same paths.

### Fixed

- Covers (Shelly 2.5 / Plus 2PM / Pro 2PM in roller mode) failed to register - confirmed working on a Shelly 2.5 in roller mode after the fix, which moves covers into the tested tier: Homebridge composed the Matter window covering without its position-aware feature and rejected the position attributes, after which every retry logged an "endpoint IDs must be unique" conflict ([#11](https://github.com/keremerkan/homebridge-shelly-matter/issues/11)). The cover now declares its configuration status so the feature is composed, and a registration rejected for a reason other than "Matter server not ready" is no longer retried - the real cause is logged once instead.
- Structural changes to an already registered accessory (an update mapping new measurements, as 0.8.0 did for Gen 1 meters) are no longer applied by re-registering the accessory in place - Apple Home can corrupt the accessory's record when a known accessory reappears with a different structure ("unable to change settings", [#8](https://github.com/keremerkan/homebridge-shelly-matter/issues/8)). The plugin now detects the change, keeps the current shape for the running session (with a log line saying so), and at the next restart of Homebridge or the child bridge removes the old accessory before the bridge is visible and adds the device back as a new accessory when it connects. The accessory lands in the bridge's default room that one time; renaming and room assignment keep working normally. Renames and firmware updates still apply immediately, in place.
- Turning power metering off for a device now re-creates its accessory cleanly at the next restart (it lands in the default room once) instead of changing the registered accessory's structure in place, for the same reason.
- Accessory identities can no longer land on a previously used identity after the accessory cache was lost or a device was hidden and un-hidden: the current identity generation is recorded in the plugin's device list and every new registration goes above it.
- Devices not seen on the network during a session (sleeping battery sensors, unplugged devices) no longer lose their row in the settings UI's device list when the device list is saved; a save pending at shutdown is flushed.
- Config entries that identify a device only by a hostname (`host` without `device`) keep applying after mDNS reports the device's IP address.
- Meters without a same-index relay were composed differently by the startup rebuild than by the live registration when power metering was off (a needless re-registration on every restart for Pro 3EM / EM class devices with metering disabled); startup rebuilds and live registrations now go through one composition engine, which also fixes a differing part order for devices whose channel numbers are not sequential (Switch Add-on).
