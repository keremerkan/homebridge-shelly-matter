...
- `accessoryType` — `light`, `outlet`, or `switch` for relay/switch channels (covers and dimmers have a fixed type). Defaults: plugs are outlets, wired relay devices are lights. On a **meter channel** (EM/3EM clamps, PM Mini), `outlet` shows the channel as a virtual outlet so Apple Home displays its live wattage on the tile (Apple shows tile wattage only for outlet types); the switch on that tile does nothing. Set it per channel, or on the device when the device has no relay channels.
