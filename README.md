...
- `vibrationAsMotion` — Door/Window sensors only: set `true` to expose the vibration (impact) detection as a motion sensor; an impact shows as motion for 10 seconds, each new impact restarts that window (Matter has no vibration sensor type, so Apple Home shows it as motion; off by default).
